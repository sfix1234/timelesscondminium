...
function describeMailConfig({ resendApiKey, fromEmail, receivingEmails }) {
  ...
  return {
    ...
    receivingCount: Array.isArray(receivingEmails) ? receivingEmails.length : null
  };
}
...
function getAgentPassword() {
  return String(process.env.AGENT_PASSWORD || '').trim();
}
...
export function verifyAgentPassword(password) {
  const configuredPassword = getAgentPassword();
...
export function verifyDirectAccessPassword(password) {
  ...
  const sessionToken = issueSignedToken({
    ...
    passwordHash: hashValue(configuredPassword),
    exp: sessionExpiresAt,
    nonce: randomBytes(16).toString('hex')
  });
...
export function getSessionRecord(sessionToken) {
  ...
  if (payload.passwordHash) {
    ...
    if (payload.passwordHash !== hashValue(configuredPassword)) {
      return null;
    }
  }
...
export async function sendAccessEmail({
  ...
  const receivingRaw =
    ...
    '';
  const receivingEmails = receivingRaw
    .split(/[,\n;\s]+/)
    .map((value) => normalizeEmail(value))
    .filter(Boolean);
  const isProduction = process.env.NODE_ENV === 'production';
  const configState = describeMailConfig({
  ...
  const cleanCountryCode = escapeHtml(countryCode);
  const cleanPhoneNumber = escapeHtml(phoneNumber);
  const cleanWealthBand = escapeHtml(wealthBand);

  if (!resendApiKey || !fromEmail) {
    if (isProduction) {
      console.error('[access-mail] missing config', configState);
  ...
  if (isProduction && /@.*resend\.dev>?$/i.test(fromEmail)) {
    console.error('[access-mail] invalid production sender', configState);
    throw createMailError(
      'mail_from_unverified',
      'ACCESS_FROM_EMAIL must use a verified domain in Resend for production delivery.'
    );
  }
  ...
  if (receivingEmails.length > 0) {
    tasks.push(
      sendResendEmail({
        ...
        from: fromEmail,
        to: receivingEmails,
        subject: '【受信通知】THE SILENCE アクセス申請',
        replyTo: email,
        html: `
          <div style="font-family: 'Hiragino Sans', 'Noto Sans JP', sans-serif; color: #111; line-height: 1.8;">
            ...
            <p>氏名: ${cleanName}</p>
            <p>メール: ${cleanEmail}</p>
            <p>国番号: ${cleanCountryCode}</p>
            <p>電話番号: ${cleanPhoneNumber}</p>
            <p>資産額: ${cleanWealthBand}</p>

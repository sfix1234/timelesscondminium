...
const IGNORED_INTERNAL_SUBJECT_PREFIXES = [
  '【受信通知】THE SILENCE アクセス申請',
  '【受信通知】THE SILENCE お問い合わせフォーム',
  '【自動返信】THE SILENCE 閲覧用パスワードのご案内',
  '【自動返信】THE SILENCE お問い合わせありがとうございます'
];
...
export function shouldIgnoreInboundEmail(email) {
  const subject = String(email?.subject || '').trim();
  return IGNORED_INTERNAL_SUBJECT_PREFIXES.some((prefix) => subject.startsWith(prefix));
}
